...
from .SHARPPYMAIN.sharppy.sharptab.utils import *
from .SHARPPYMAIN.sharppy.sharptab.constants import *
from .SHARPPYMAIN.sharppy.sharptab.thermo import *
...
from .SHARPPYMAIN.sharppy.io.decoder import Decoder
...
class ARWDecoder(Decoder):
    ...
    def _parse(self):
        ...
        if len(file_data.variables["T"][:].shape) == 4:        
            ...
            mtmpc = theta(1000.0, theta - 273.15, p2=mpres) 
            ## convert the mixing ratio to dewpoint
            mdwpc = temp_at_mixrat(qvapr, mpres)
        ...
        for i in range(file_data.variables["T"][:].shape[0]):
            ...
            prof = create_profile(profile="raw", pres=prof_pres, 
        ...
        prof_coll = ProfCollection({'':profiles}, dates)
